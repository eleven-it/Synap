...
class AdministraNETConfigAdmin(admin.ModelAdmin):
    list_display = ['name', 'database', 'is_active', 'created_at']
    list_filter = ['is_active', 'created_at']
    ...
    def save_model(self, request, obj, form, change):
        ...
        be = get_session_base_empresa(request)
        if be:
            obj.database = be
    ...
    fieldsets = (
        ...
        (_('Esquema MySQL (pool Synap)'), {
            'fields': ('database',),
            'description': _('Host y credenciales provienen de DATABASES["mysql"] en Synap.'),
        }),
        (_('Integración Tienda Nube'), {
            'fields': (
