...
class TiendanubeConfig(models.Model):
    ...
    location_id = models.PositiveBigIntegerField(
        ...
        help_text=_(
            "ID de ubicación Tienda Nube para inventory_levels; "
            "vacío omite location_id del payload de stock."
        ),
    )
...
class TiendanubeOutboxEvent(models.Model):
    """
    Cola saliente Adminet → Tienda Nube (stock/precio, catch-up pedidos).

    Separada de WebhookEvent (inbound). Semántica persistir → worker.
    """

    class EventType(models.TextChoices):
        STOCK_PUSH = 'stock_push', _('Stock Push')
        CATCH_UP_ORDERS = 'catch_up_orders', _('Catch Up Orders')
    ...
    class Meta:
        verbose_name = _('Tiendanube Outbox Event')
        verbose_name_plural = _('Tiendanube Outbox Events')
        ordering = ['created_at']
        indexes = [
    ...
    def mark_failed(self, error_message: str, *, retry: bool = False):
        self.error_message = error_message
        self.retry_count += 1
        if retry and self.retry_count < self.max_retries:
            ...
            self.status = self.EventStatus.FAILED
        self.save(
            update_fields=['status', 'error_message', 'retry_count', 'next_retry_at']
        )
