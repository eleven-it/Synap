"""
Drenar inbox webhook Tienda Nube (pending + retry vencido).
...
Cuando ops reactive Celery, usar la task ``drain_webhook_inbox`` en
...
class Command(BaseCommand):
    help = (
        'Drena eventos webhook pending/retry vencidos (batch default 50). '
        'Cron sin Celery: */1 * * * * docker exec Synap_app '
        'python manage.py tiendanube_drain_inbox'
    )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f'Drenaje inbox: {result.processed} procesados, '
