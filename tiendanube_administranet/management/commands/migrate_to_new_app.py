            ...
            for old_config in old_configs:
                try:
                    if not dry_run:
                        ...
                        if existing:
                            # Actualizar configuración existente
                            existing.name = old_config.name or f"Migrated {old_config.host}/{old_config.database}"
                            existing.port = old_config.port
                            existing.user = old_config.user
                            existing.password = old_config.password
                            existing.is_active = old_config.is_active
                            existing.save()
                        else:
                            # Crear nueva configuración
                            new_model.objects.create(
                                name=old_config.name or f"Migrated {old_config.host}/{old_config.database}",
                                host=old_config.host,
