...
class AdministraNETConfigForm(forms.ModelForm):
    ...
    def __init__(self, *args, request=None, **kwargs):
        ...
        try:
            from .mysql import get_session_base_empresa, resolve_mysql_base_empresa
            from .services.adminet_service import AdministraNETService

            session_be = get_session_base_empresa(self._request)
            adminet_config = AdministraNETConfig.objects.filter(is_active=True).first()
            if not adminet_config and session_be:
                adminet_config = AdministraNETConfig(database=session_be)

            if adminet_config:
                try:
                    be = resolve_mysql_base_empresa(self._request, adminet_config)
                except Exception:
                    be = (adminet_config.database or "").strip() or session_be
            ...
            self.fields['punto_venta_tiendanube_choice'].choices = [('', _('Error cargando puntos de venta...'))]
    ...
    def save(self, commit=True):
        ...
        be = get_session_base_empresa(self._request)
        if be:
            instance.database = be
    ...
    def clean(self):
        ...
        if self._request and not get_session_base_empresa(self._request):
            raise ValidationError(
                _(
                    'No se detectó la empresa en sesión (base_empresa). '
                    'Inicie sesión de nuevo o seleccione empresa en Synap.'
                )
            )
