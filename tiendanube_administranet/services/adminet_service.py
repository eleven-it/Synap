...
Usa el pool MySQL de Synap (``core.mysql_pool.get_connection``), alineado con
RequestScopedMysqlMiddleware y las credenciales de ``settings.DATABASES['mysql']``.
La base de datos efectiva es ``base_empresa`` (nombre de esquema), alineada con la sesión.
"""
...
class AdministraNETService:
    ...
    def __init__(self, config: AdministraNETConfig, base_empresa: Optional[str] = None):
        self.config = config
        self._base_empresa = (base_empresa or (config.database or "")).strip()
        if not self._base_empresa:
            raise ValueError(
                "Indique base_empresa o configure AdministraNETConfig.database (empresa MySQL)."
            )

    def test_connection(self) -> Dict[str, Any]:
        """Probar conexión con AdministraNET vía pool Synap."""
        try:
            ...
            logger.error("Error testing AdministraNET connection: %s", e)
            return {
                'success': False,
    ...
    def execute_query(self, query: str, params: Optional[tuple] = None) -> Dict[str, Any]:
        """Ejecutar consulta SQL usando el pool MySQL de Synap."""
        try:
            q0 = query.strip().upper()
            is_read = q0.startswith(('SELECT', 'SHOW', 'DESCRIBE', 'EXPLAIN'))
            with get_connection(self._base_empresa) as conn:
                ...
                return {
                    ...
                    'last_insert_id': int(last_id) if last_id else None,
    ...
    def verify_and_migrate_schema(self) -> Dict[str, Any]:
        ...
        Delega en ``core.services.legacy_mysql_schema.catalog.run_tiendanube_integration_mysql``.
        """
        try:
            with get_connection(self._base_empresa) as conn:
                return run_tiendanube_integration_mysql(conn)
        except Exception as e:
            logger.error("Error verificando schema: %s", e)
            return {
                "success": False,
                "message": f"Error verificando estructura: {str(e)}",
                "migrations_applied": [],
                "migrations_failed": [str(e)],
            }
        ...
        try:
            ...
            if result['success']:
                ...
                if not new_id:
                    return {
                        'success': False,
                        'message': 'Producto insertado pero no se obtuvo LAST_INSERT_ID',
                    }
                return {
                    'success': True,
    ...
    def close_connection(self):
        """Compatibilidad: las conexiones las gestiona el pool MySQL de Synap."""
        return None
