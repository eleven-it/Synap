...
class OsTicketChatIntegration:
    ...
    def create_ticket_from_chat(
        ...
        Args:
            ...
            agent_id: ID del agente que procesó la consulta
        """
        try:
            ...
            insert_query = """
                INSERT INTO ost_ticket (
                    user_id, status_id, priority_id, dept_id, topic_id, 
                    created, updated, source, source_extra
            ...
            if conversation_context and len(conversation_context) > 1:
                ...
                note_query = """
                    INSERT INTO ost_note (
                        pid, user_id, staff_id, type, title, body, created
                    ) VALUES (%s, %s, %s, %s, %s, %s, %s)
                """
            ...
            return {
                'success': True,
                'ticket_id': ticket_id,
                'ticket_number': self._get_ticket_number(ticket_id),
                'status': 'abierto',
                'priority': self._get_priority_name(ticket_data['priority_id']),
                'department': self._get_department_name(ticket_data['dept_id']),
                'message': f'Ticket creado exitosamente. Número: {self._get_ticket_number(ticket_id)}'
            }
    ...
    def add_response_to_ticket(
        ...
        Args:
            ...
            staff_id: ID del staff (si es respuesta del equipo)
            response_type: Tipo de respuesta (R=Respuesta, N=Nota)
        """
        try:
            conn = self.osticket_integration.get_connection()
            if not conn:
            ...
            entry_query = """
                INSERT INTO ost_thread_entry (
                    thread_id, staff_id, user_id, type, title, body, format, created
    ...
    def _get_or_create_user(self, cursor, user_id: str, user_email: str) -> int:
        ...
        cursor.execute("SELECT id FROM ost_user WHERE email = %s", (user_email,))
        ...
        # Crear nuevo usuario
        cursor.execute("""
            INSERT INTO ost_user (email, created) VALUES (%s, %s)
        """, (user_email, timezone.now()))
    ...
    def _get_priority_name(self, priority_id: int) -> str:
        ...
        return priority_names.get(priority_id, 'Baja')
