...
4. **Header auth:** Mantener `Authentication: bearer {token}` (funciona en producción; distinto del ejemplo docs `Authorization: Bearer`). Detalle y smoke test: **`docs/ecom/ADR_TIENDANUBE_CONTRATOS_API_2026.md`** §2.
5. **Contratos payload 2026 (sin bump):** `visibility` XOR `published`; stock con `inventory_levels` + `location_id` opcional; HTTP **402** → NOT CONFIGURED. Ver **`docs/ecom/ADR_TIENDANUBE_CONTRATOS_API_2026.md`**.
6. **Webhooks HMAC:** Cabecera `x-linkedstore-hmac-sha256`, secret en `TiendanubeConfig.webhook_secret`.
7. **Rate limit:** 2 req/s + headers `x-rate-limit-*` + reintentos 429 vía `services/rate_limit.py` en `TiendanubeService._request`. Ver también sync inicial: `docs/ecom/TIENDANUBE_CUSTOMER_MAPPING_HARDENING.md` § Sync inicial por lotes.
...
- `docs/ecom/ADR_TIENDANUBE_CONTRATOS_API_2026.md`
- `docs/ecom/TIENDANUBE_WEBHOOKS_API_2025-03.md`
- `docs/ecom/TIENDANUBE_DEUDA_TECNICA_P0_P2.md`
- `docs/ecom/TIENDANUBE_REFLOTE_DEUDA_PENDIENTE.md`
