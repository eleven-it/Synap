...
2. **Hero oscuro (una fila):** título + badge «Borrador #N»; acciones a la derecha: **vendedor operativo** (antes de Anular), Anular, Hub y **Confirmar pedido** (`bg-orange-500`, mismo naranja CTA Synap del navbar). El selector de vendedor se muestra si el usuario ve más de un viajante o tiene `ecom.pedidos.ver_todos` / puesto Administracion-Supervisor: cambiarlo **actualiza** el borrador (territorio VCM), no lo borra. Tras confirmar (o PED en solo consulta) aparece **Nuevo** (dropdown purple: Pedido simple | Masivo sucursales), igual que en el Hub. Sin subtítulo de packs/autoguardado.
...
5. Filas = artículos **Terminado** de marcas asignadas con **paridad carrito/precio**: `Discontinuo='No'` y `ecommerce='Si'` (mismo criterio que `obtener_articulo_row_precio` / `agregar_item`). El buscador predictivo (`buscar_articulos_filtrados_ternas`) no ofrece ítems que luego fallarían en preview/confirm con «Artículo no encontrado o inactivo». Criterios de búsqueda tipada (≥2 caracteres, `tam=20`): código de sistema (`IDArt`/`CodigoArticuloT`), `id_manual`, nombre y **código de barra** (`NroCodBarra`). **Flecha abajo** (o botón ▾) lista **todo** el catálogo filtrado (`?todos=1`, sin mínimo de 2 caracteres; tope 5000). Al navegar con ↑/↓ el ítem resaltado hace `scrollIntoView({ block: 'nearest' })` dentro del dropdown (`max-h-56`), mismo patrón que el catálogo mayorista. Desktop y móvil.  
   Columna **Precio** = precio de la fila (precargado de la lista; editable si el puesto tiene `mod_precio_pedido` o es supervisor). Ese valor es el que se confirma en el PED. Columna **STOCK** = packs disponibles Terminado (ver § stock abajo).  
...
7. **Fecha de entrega** obligatoria al confirmar: si falta, modal de aviso y foco en el campo. En el **alta** (primera edición, sin fecha previa) se prellena **vencimiento + 10 días**; si cae sábado o domingo, el **lunes inmediato**. Si ya venía del PED o el usuario la cargó, se conserva (no se recalcula al cambiar vencimiento/condición). Cliente en UI sin `(cod: N)`.  
   **Lista / condición / descuentos / precio de línea:** el supervisor de venta siempre puede editarlos. Un vendedor sin ese rol los edita solo si su puesto tiene en **Archivo → Permiso en sistema** `mod_lista_de_precio`, `cambia_cv`, `mod_descuento_pie`, `mod_descuento_renglon` y/o `mod_precio_pedido`. El vencimiento manual sigue siendo solo supervisor. Si se cambia la lista, un modal Synap pregunta si se **recalculan** los precios o se **conservan** los ya cargados. Precio 0 en una línea con cantidad alarma y no deja validar/confirmar.  
8. Confirmar → **1 PED por sucursal** con `cliente_datos_adicionales.id_cliente_domicilio`.
9. **Importar Excel (19/08/2026, `?v=masivo45`):** con borrador editable, la tarjeta de contexto ofrece **Descargar plantilla** e **Importar Excel**. La plantilla **v5** trae **una fila por SKU** (`IDArt` en columna C oculta; SuperArt/`id_manual` en A no es único). Columna **D = Precio** (unitario neto de la fila); sucursales desde **E**. Quien tiene `mod_precio_pedido` (o es supervisor) importa cantidades **y** precios de **ese** borrador. Precio 0 con cantidad → error `precio_cero` y el draft no cambia. Precio vacío → se usa el de lista. Plantilla v4 (sin precio): solo cantidades; el precio sale de la lista. v3 (sin `IDArt`): desambigua por nombre. Tope de filas: 5000. Ids de sucursal y cliente van en hoja `_Synap` (veryHidden). Si no coinciden con el pedido abierto → HTTP 409. **Reemplazo total** de celdas; pie ← `cliente.Descuento` y renglón ← `descuento_por_cli`. El import **no** valida stock. Modal Synap (`masivo_importar`); sin `alert`/`confirm`.
...
| GET | `/ecom/api/mayoristapp/pedido-masivo/plantilla-excel/?draft_id=` | Descarga `.xlsx` v4: una fila por SKU (SuperArt + nombre + `IDArt` oculto) y sucursales desde D; cantidades editables; identifica cliente |
