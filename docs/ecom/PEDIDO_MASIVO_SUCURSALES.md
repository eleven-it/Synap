...
7. **Fecha de entrega** obligatoria al confirmar: si falta, modal de aviso y foco en el campo. En el **alta** (primera edición, sin fecha previa) se prellena **vencimiento + 10 días**; si cae sábado o domingo, el **lunes inmediato**. Si ya venía del PED o el usuario la cargó, se conserva (no se recalcula al cambiar vencimiento/condición). Cliente en UI sin `(cod: N)`.  
   **Lista / condición / descuentos:** el supervisor de venta siempre puede editarlos. Un vendedor sin ese rol los edita solo si su puesto tiene en **Archivo → Permiso en sistema** `mod_lista_de_precio`, `cambia_cv`, `mod_descuento_pie` y/o `mod_descuento_renglon`. El vencimiento manual sigue siendo solo supervisor.  
8. Confirmar → **1 PED por sucursal** con `cliente_datos_adicionales.id_cliente_domicilio`.
9. **Importar Excel (13/08/2026, `?v=masivo44`):** con borrador editable, la tarjeta de contexto ofrece **Descargar plantilla** e **Importar Excel**. La plantilla v4 trae **una fila por SKU** (`IDArt` en columna C oculta; SuperArt/`id_manual` en A no es único — varios colores pueden compartir código). Columnas de sucursal desde D; **solo hay que completar cantidades** (sin precios ni descuentos). Encabezado visible (nro + calle, fila alta); **sin protección de hoja** para poder seleccionar columnas. Plantillas v3 (sin `IDArt`): el import desambigua por nombre (columna B) y prioriza `id_manual` frente a colisión con `IDArt`. Tope de filas: 5000 (en `administranet` prod al 13/08/2026 hay 310 Terminado con ecommerce). Ids de sucursal y cliente van en hoja `_Synap` (veryHidden). Al subir, si no coinciden con el pedido abierto → HTTP 409 y el borrador no cambia. **Reemplazo total** de celdas; pie ← `cliente.Descuento` y renglón ← `descuento_por_cli`. El import **no** valida stock (sí preview/confirmar). Sin cantidades, `.csv`, Excel corrupto o sin hoja `Pedido` → no se modifica el borrador. Modal Synap (`masivo_importar`); sin `alert`/`confirm`.
