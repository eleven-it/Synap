...
**Actualización:** 2026-04-30 — Barra de tabla (`logistica_lista_comprobantes_rutas_tabla_toolbar.html`): se elimina el enlace «Abrir módulo Entregas (operación en Synap)»; el acceso operativo sigue por menú **Logística → Entregas**.
**Actualización:** 2026-04-30 — Alineación tabla con agrupación: ``colgroup`` + ``table-layout: fixed`` en la tabla del informe y en la tabla anidada por grupo; claves de columnas unidas desde **todas** las filas del dataset (no solo ``data[0]``) para que el número de columnas coincida siempre con el thead.
**Actualización:** 2026-04-30 — Tabla del informe: columnas ocultas en UI (siguen en API/SQL para modales y export): **Nº pedido**, **Orden en ruta**, **Franja horaria (ruta)**; la búsqueda en tabla sigue indexando esos campos.
