...
| `seleccionarComprobante` | GET | `/ecom/api/mayoristapp/clientes/comprobante-formulario/?ajax=1&frm=0..5` | Respuesta `{ "estado":"ok", "url", "formulario" }`; guarda en `mayoristapp.formulario` / `u_formulario`. **`frm=0` (pedido):** URL Synap `/ecom/mayoristapp/compra/` (antes `alta_pedido.php`). Ver [SPEC_GESTION_PEDIDOS_SYNAP.md](./SPEC_GESTION_PEDIDOS_SYNAP.md). |
