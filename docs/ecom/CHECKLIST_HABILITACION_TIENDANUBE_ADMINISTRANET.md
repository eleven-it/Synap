...
## Post-reflote — checklist activación ops (2026)

Documentación del change **`tiendanube-administranet-reflote`**.  
**No ejecutar en Desarrollo salvo entorno de prueba dedicado** — estos pasos son para staging/producción cuando producto autorice go-live.

Deuda y contexto: **[TIENDANUBE_REFLOTE_DEUDA_PENDIENTE.md](TIENDANUBE_REFLOTE_DEUDA_PENDIENTE.md)**  
Contratos API: **[ADR_TIENDANUBE_CONTRATOS_API_2026.md](ADR_TIENDANUBE_CONTRATOS_API_2026.md)**

### 1. Migraciones Django (obligatorio)

Aplicar migraciones del módulo en PostgreSQL (`default`):
...
|-----------|------------|
| `0026_tiendanubeconfig_location_id` | `TiendanubeConfig.location_id` opcional (multi-inventario TN) |
| `0027_tiendanubeoutboxevent` | Outbox saliente stock/precio/catch-up + índice retry |

### 2. Activar `ModuleConfig` del módulo

El código del reflote **no** activa el módulo automáticamente. Pasos recomendados:

```bash
# Registrar o actualizar metadata del módulo en ModuleConfig
docker exec Synap_app python manage.py add_tiendanube_administranet_module

# Activar módulo, menús y permisos de rutas
docker exec Synap_app python manage.py setup_modules --activate tiendanube_administranet --reload-menus
```

Verificar en BD: `ModuleConfig.name = 'tiendanube_administranet'` con `is_active = True`.  
Confirmar entrada en `APPS_MENU` (`core/utils/utils.py`, id `tiendanube_administranet`).

### 3. Cron interim — drenaje inbox y outbox (sin Celery)

Mientras **`django_project/celery.py`** permanezca comentado, programar **cada 1 minuto**:
...
- Batch default: 50 eventos (`--limit` opcional).
- Monitorear exit code ≠ 0 (fallos en el lote).
- Sin estos cron, webhooks quedan en `pending` y el outbox no empuja stock/precio.

### 4. Credenciales y configuración operativa

Completar en UI (`/tiendanube_administranet/`) **antes** de tráfico real:

| Ítem | Dónde / notas |
|------|----------------|
| **Access token** | `TiendanubeConfig` — wizard o edición; vacío en edición no sobrescribe |
| **Store ID** | Obligatorio; wizard persiste al guardar tienda |
| **HMAC / webhook secret** | `TiendanubeConfig.webhook_secret` = App Secret Nuvemshop; **obligatorio** en `ENVIRONMENT=production` |
| **Depósito TN** | `AdministraNETConfig.deposito_tiendanube_id` — **requerido** si `sync_stock` |
| **Punto de venta** | `AdministraNETConfig.punto_venta_tiendanube_id` — pedidos `comp_ped` |
| **`location_id` TN** | Opcional en `TiendanubeConfig` — multi-inventario; vacío omite campo en payload |
| **URL webhook pública** | `{SITE_URL}/tiendanube_administranet/webhook/` — HTTPS en prod |

Probar: «Test connection» Tienda Nube y AdministraNET; registrar webhooks (`order/paid`, LGPD).

### 5. Cuando ops active Celery + Redis

**No descomentar en el merge del reflote** — acción exclusiva de ops en el entorno:

1. Habilitar Celery en `requirements.txt` y descomentar `django_project/celery.py`.
2. Descomentar en `tiendanube_administranet/tasks/webhook_tasks.py` el bloque Beat:
   - `drain-webhook-inbox-every-60s` → task `drain_webhook_inbox`
   - `drain-outbox-every-60s` → task `drain_outbox`
3. Levantar worker + beat; **retirar** cron duplicado de §3.
4. Validar drenaje con eventos `pending` de prueba.

### 6. Verificación post-activación

- [ ] POST webhook simulado → `WebhookEvent` `pending` → cron/task → `completed`
- [ ] Pedido `order/paid` → `OrderMapping` + `comp_ped` sin duplicar
- [ ] Cambio stock Adminet → `TiendanubeOutboxEvent` → push TN con `inventory_levels`
- [ ] HTTP 402 en API TN → log NOT CONFIGURED, **sin** retry transitorio
- [ ] Kill switch `.env` probado: `TIENDANUBE_WEBHOOKS_ENABLED`, `TIENDANUBE_SYNC_ENABLED`
