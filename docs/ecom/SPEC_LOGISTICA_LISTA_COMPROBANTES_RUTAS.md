...
- **Tabla:** primera columna visible **Fecha factura** (`factura.Fecha`, formato `dd/MM/yyyy`); el listado sigue exponiendo `fecha_remito` para otros consumidores (p. ej. módulo Entregas). **Agrupar por:** opción **Mes de factura** agrupa por `mes_factura_ym` (`YYYY-MM`) y muestra cabeceras de grupo solo como mes y año en mayúsculas (p. ej. «ABRIL 2026»), sin prefijo «MES DE FACTURA:».
- **Columnas no mostradas en la grilla** (siguen en la respuesta del listado para export y modales): **Nº pedido** (`nro_pedido`), **Orden en ruta** (`orden_ruta`), **Franja horaria (ruta)** (`ventana_horaria_ruta`). La búsqueda en tabla (≥2 caracteres) **sí** considera esos campos además de las columnas visibles.
