...
**Estabilidad mobile vs desktop (marzo 2026):** Para evitar que el layout "cambie" a desktop escalado tras cargar, se introdujo la clase `tpv-mobile` en el body cuando se sirve el template mobile: en `mobile/kiosco.html` se sobreescribe `{% block content %}` para añadir `document.body.classList.add('tpv-mobile')` junto a `kiosk-client-mode`. Todos los overrides CSS del template mobile usan el selector `body.tpv-mobile` (o `body.tpv-mobile .kiosk-col-right`, etc.) para que tengan igual o mayor especificidad que las reglas del desktop (`body.kiosk-client-mode .kiosk-col-right { max-height: none }` y `@media (min-width: 1024px) .kiosk-main-two-cols`). Así, cuando el usuario entra al TPV desde móvil, el layout permanece en columna única y carrito tipo bottom sheet. Además, todos los templates mobile del módulo (selector_kiosco, config_list, carritos_pendientes, talonarios_list) incluyen `{% block extra_meta %}` con el mismo viewport para que toda la navegación en mobile tenga escala correcta.
