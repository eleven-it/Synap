...
- **docs/reports/ANALISIS_FORMULARIOS_STOCK_VB6.md** — Formularios de stock/inventario (CargaMovStock, Stock, Inventario, etc.) y relación con stock_deposito.
- **docs/general/INFO_COMPRA_TABLAS_CAMPOS.md** — Flujo compras (OC → REM → Factura) y escritura en stock/stock_deposito.
- **docs/general/CONTEXTO_TABLAS_VB6_INFORMES.md** — Contexto de tablas para informes (stock, stock_deposito, stockp).
...
*Elaborado a partir de la búsqueda en administranet_vb6/Formularios (rs_stock, AddNew, Comprobante, TipoComp, Entrada, Salida, anulado) y de la documentación existente en docs (docs/reports, docs/general, docs/self_checkout).*
