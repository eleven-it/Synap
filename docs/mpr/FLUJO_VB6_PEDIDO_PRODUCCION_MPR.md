...
5. **Orden y agrupación**
   - En Synap, `listar_ventana_pack` agrupa por `id_articulo` sumando `cantidad_pendiente_prod` de las filas de `lista_produccion_agrupada` con demanda pendiente (`en_proceso_produccion = 'No'`, pendiente > 0), **excluyendo** filas con `codigo_movimiento_opt > 0` (líneas de OPT ya liberada a MSTOCK), para no duplicar demanda con restos de OPT cerradas. Varias líneas de pedido pueden seguir sumando el mismo artículo. En VB6 la grilla global también puede ser por artículo. Si en VB6 hay una agrupación distinta (p. ej. por id_lista_produccion y no por artículo), los totales por artículo pueden no coincidir.
