...
- **Menú:** MPR añadido a `APPS_MENU` en `core/utils/utils.py` (id `mpr`, orden 4, permiso `mpr.ver`), con submenús: Tablero, Demanda (Pedido producción trabajo (OPT)/Unidades), Órdenes de producción (Lista de OP, Nueva OP), Ejecución (Liberar OPT, Armado Lista de materiales, OPP), Configuración. Módulo incluido en `core_modules` para visibilidad sin depender de `ModuleConfig`.
