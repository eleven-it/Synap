...
| Layout viewport | **Parte y Control de calidad:** **`-mt-4 md:-mt-8`** cancela el padding superior de `base_app`. Altura **`h-[calc(100dvh-6.5rem)] md:h-[calc(100dvh-9rem)]`** (holgura para paddings + status; evita scroll de `body`). Chrome **`flex-shrink-0` sin sticky** (el `sticky z-[60]` tapaba el navbar fijo `z-[60]` y el thead). Zona de datos **`flex-1 min-h-0 overflow-hidden`**; grilla scrollea dentro; thead de CC **`sticky top-0`** con fondos opacos; footer «Guardar» **`flex-shrink-0`**. Tablero: `100dvh-4.5rem` + chrome sticky `z-40`. |
