...
| Layout viewport | **Parte y Control de calidad:** **`-mt-4 md:-mt-8`** cancela el padding superior de `base_app` (chrome pegado al navbar, como Reports) y **`h-[calc(100dvh-5.5rem)] md:h-[calc(100dvh-7.5rem)]`** deja margen para status bar + footer «Guardar». Chrome **`flex-shrink-0`**, zona de datos **`flex-1 min-h-0 overflow-hidden`**. La grilla scrollea dentro; footer «Guardar» (borrador + confirmar) es **`flex-shrink-0`** al pie de la tarjeta — siempre visible sin scrollear la página. Chrome **`sticky top-14 md:top-16 z-[60]`**. Tablero: `100dvh-4.5rem` + chrome `z-40` (sin footer Guardar de captura). |
