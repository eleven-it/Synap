...
| **Control de calidad (clasificación / CC)** | Clasificación desde **Producido** hacia Semi / 2da / Scrap, por **operario fabricante**. Tope fila = remanente del parte + **extra producción** (`stock Prod − Σ atribuible parte`). Ledger: `mpr_transicion_lote` (`cantidad_extra`). Ruta: `/mpr/clasificacion-produccion/`. No confundir con la planilla impresa. |
