...
| **Control de calidad (clasificación / CC)** | Clasificación desde **Producido** hacia Semi / 2da / Scrap, por **operario fabricante**. Tope fila = remanente atribuible del parte + **extra producción** (`stock Prod − Σ atribuible parte`). Semi precarga con el atribuible del Parte; al cargar 2da o Scrap la UI descuenta el delta de Semi (semi sigue editable a mano). Ledger: `mpr_transicion_lote` (`cantidad_extra`). Ruta: `/mpr/clasificacion-produccion/`. No confundir con la planilla impresa. |
