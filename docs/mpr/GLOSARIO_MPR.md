...
| **Control de calidad (clasificación / CC)** | Clasificación desde **Producido** hacia Semi / 2da / Scrap, por **operario fabricante**. Tope fila = remanente atribuible del parte + **extra producción** (`stock Prod − Σ atribuible parte`). Semi/2da/scrap **arrancan en 0** (sin precarga del parte); el usuario completa celda por celda. Borrador y filas confirmadas (solo lectura) sí muestran el desglose guardado. Al cargar 2da o Scrap la UI descuenta el delta de Semi si ya había cantidad. Ledger: `mpr_transicion_lote` (`cantidad_extra`). Ruta: `/mpr/clasificacion-produccion/`. No confundir con la planilla impresa. |
