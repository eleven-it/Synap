...
|-------|---------------------|--------|
| **articulo** | `IDArt` (PK), `CodigoArticulo`, `CodigoArticuloT`, `NombreArticulo`, `id_en_abm`, `ensamblado` ('Si'/'No'), `stock_reserva` (opc.), `stock_minimo` (opc.), **`cantidad_promedio_bulto`** (opc.) | Artículo puede ser "armado" si `ensamblado='Si'` y `id_en_abm` no nulo. En **Pedido producción trabajo (OPT)** las columnas «Docenas» (pack, pestaña Unidades/BOM y Confirmar OPT) calculan unidades ÷ `cantidad_promedio_bulto`; si el valor es ≤ 0 o la columna no existe, Synap usa **12** como divisor (`docenas_desde_unidades_opt` en `mpr/services.py`). La lectura del nombre de columna en MySQL es **case-insensitive** (`nombre_columna_ci` en `core/services/legacy_mysql_schema/helpers.py`) para no perder el dato si el legado difiere en mayúsculas. En la **pestaña Unidades (BOM)** el saldo del componente es **solo** `stock_deposito` del depósito con `deposito.tipo_mpr='SemiElaborado'` (no el agregado `suma_stock` del pack). |
