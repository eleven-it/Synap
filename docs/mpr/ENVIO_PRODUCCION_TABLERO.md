...
| Terminado\|A Fabricar y Docenas\|Pares | Solo con `mpr.tablero_cambiar_vista` (opt-in; no lo incluye `mpr.ver`) | Idem | — |
...
Perfil **operario + tablero:** `mpr.parte_operario` + `mpr.tablero_ver`, sin `mpr.ver`. Landing en `/mpr/mi-parte/`; consulta demanda en el tablero sin mutar ledger ni abrir CC/reportes. Flags de contexto: `puede_enviar=False`, `solo_lectura_tablero=True`. Sin `mpr.tablero_cambiar_vista` no ve Terminado\|A Fabricar ni Docenas\|Pares (queda A Fabricar / Docenas).
