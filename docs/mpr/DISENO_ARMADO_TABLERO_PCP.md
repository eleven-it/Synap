...
| 15 | **Panel «Ya armado» (03/08/2026):** tarjeta sky colapsable **mismo patrón que Contexto comercial** (pedido masivo: eyebrow + chevron `expand_more` con `-rotate-90`); compacto por defecto con resumen Fecha/Artículos/Packs; expandido = tabla Artículo/Packs consolidada por `id_articulo_pack` (sin comprobante ni código). Chrome oscuro sin botón extra. Default presentación **Pares** en Armado. |
