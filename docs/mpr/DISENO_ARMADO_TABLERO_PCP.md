...
| 15 | **Panel «Ya armado» (03/08/2026):** tarjeta sky colapsable **mismo patrón que Contexto comercial**; chrome 1 línea (`flex-nowrap`): sin Tablero con texto ni Imputación; búsqueda compacta; separación visual (borde) antes de la grilla. Consolida por `id_articulo_pack`. Default **Pares** en Armado. |
| 16 | **Flujo del chrome (05/08/2026):** en la página con altura fija y scroll interno de grilla, el chrome y «Ya armado» permanecen en el flujo normal; no usar `sticky` de viewport. Solo el `thead` y las columnas izquierdas son sticky dentro del `overflow-auto` de la tabla, para no ocultar sus encabezados. |
