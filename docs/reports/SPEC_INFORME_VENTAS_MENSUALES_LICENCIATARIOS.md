...
**Panel Preview QA:** además de clientes seed pendientes de match, muestra la lista de **SuperArt desconocidos** (`qa_superarts`) cuando el pack Puma encuentra códigos fuera del catálogo activo. Botón **Clasificar SuperArt** abre modal Synap (`modal_licenciatarios_superart_qa.html`): pestañas **Pendientes** (desde `MonthlyReportingSuperArtQAPending` + códigos del último preview) y **Catálogo** (entradas del catálogo activo, con filtro Men/Women y búsqueda por código); por ítem botones **Men** / **Women** para clasificar o editar género; al guardar se agrega o actualiza la entrada en el catálogo activo (crea versión 1 si no hay activa), elimina el pending si correspondía y actualiza badge/lista. Toast con `mprShowAviso` / `SynapMessages`. Permisos: lectura `OperationalReportsPermission` (`reports.view_operational`); escritura `reports.licenciatarios_clasificar_superart` (incluye puesto **Supervisor**, `reports.*`, usuario `cod_usuario=supervisor` / admin). El match de clientes seed sigue exigiendo `user_has_full_access`.
