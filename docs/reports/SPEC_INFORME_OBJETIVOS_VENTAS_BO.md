...
- **Dataset jerárquico** (contrato sugerido para `meta.extra`):
  ...
  - Bajo cada cliente, opcionalmente **`venta_detalle`**: árbol **rubro → subrubro → artículo** con agregados de **`cantidades_vendidas`** y **`facturacion`** por nodo (misma ventana y filtros que la consulta de unidades del informe; importe por renglón coherente con criterio de ventas netas). Las mismas filas llevan **backorder en importe** desglosado por ítem (`stockp` + `articulo` → rubro/subrubro), **sumado en cada nivel** (artículo → subrubro → rubro); coincide con el total BO del cliente salvo líneas sin `IDArt` válido (solo van al total cliente). **Remitos**, **pedidos en armado** y **total** consolidado **no** se desgliegan al árbol: en rubro/subrubro/artículo la tabla web muestra **—** en **REMITOS**, **PEDIDOS EN ARMADO** y **TOTAL** (solo el renglón **cliente** lleva importes reales). **Objetivo** y **Falta** siguen solo a nivel cliente/vendedor (— en rubro/subrubro/artículo). La UI replica expansión/colapso por vendedor, cliente, rubro y subrubro (persistencia en `localStorage` bajo clave `synap:report-view:ventas-objetivos-vs-bo:jerarquia`). Cabeceras en **mayúsculas**; grupo **VENTAS PERÍODO** con `colspan="5"` sobre **UNIDADES**, **FACTURACIÓN**, **REMITOS**, **PEDIDOS EN ARMADO** y **TOTAL**; columnas **OBJETIVO** (fondo verde suave) y **FALTA** (fondo rojo suave) con `rowspan="2"`; subcabecera **TOTAL** con tono violeta un poco más marcado; grupo **BACKORDER** con **BO TOTAL** al final más oscuro; fila **Totales** resaltada.
