...
| Falta | **`Falta = Objetivo − Facturación − Remitos − Pedidos en armado`**. Facturación y remitos en la **ventana de facturación** del informe; **pedidos en armado** = saldo de `comp_ped` **PED** en estado **En preparación** / **Preparado** (**sin filtro de fecha**, igual que el KPI «Pedidos en armado» de **`total-consolidado-operativo`**). |
| Pedidos en armado | Por **cliente**: suma de **`SubtotalDesc`** en `comp_ped` con los filtros de sucursales / clientes excluidos / vendedores excluidos alineados al resto del informe. Campo JSON: **`pedidos_en_armado`**. |
| Total (columna; nombre de campo **`total`**) | **`Total = Facturación + Remitos + Pedidos en armado`** (total consolidado por fila cliente, misma composición que el total consolidado operativo a nivel empresa si se sumaran todas las filas bajo los mismos filtros). |
...
- **Filtros**: mismos que `bo-stock-facturacion` para período dual, sucursales, **punto(s) de venta** (`punto_venta` → `cc.id_pv` / `cp.id_pv` en facturación, remitos, pedidos en armado, unidades/detalle y BO), clientes excluidos, depósitos, lista de precio donde aplique al BO.
- **Dataset jerárquico** (contrato sugerido para `meta.extra`):
  ...
  - Lista de **hijos** (cliente): `codigo_cliente`, `nombre_cliente`, `objetivo`, `facturacion`, `remitos`, `pedidos_en_armado`, `total`, `falta`, `cantidades_vendidas`, columnas BO (total y, si aplica, con stock / con ingreso / sin stock) en **importe**, alineadas al criterio del BO actual por cliente.
  - Bajo cada cliente, opcionalmente **`venta_detalle`**: árbol **rubro → subrubro → artículo** con agregados de **`cantidades_vendidas`** y **`facturacion`** por nodo (misma ventana y filtros que la consulta de unidades del informe; importe por renglón coherente con criterio de ventas netas). Las mismas filas llevan **backorder en importe** desglosado por ítem (`stockp` + `articulo` → rubro/subrubro), **sumado en cada nivel** (artículo → subrubro → rubro); coincide con el total BO del cliente salvo líneas sin `IDArt` válido (solo van al total cliente). **Desglose por líneas REM/PED:** en rubro/subrubro/artículo el backend puede enviar **`remitos_lineas`** y **`pedidos_armado_lineas`** (suma `stockp.PrecioNetoxR` alineada a `verify_objetivos_remitos_ped_lineas_vs_cabecera`: REM en ventana de facturación; PED en armado sin filtro de fechas; exclusión `tipo_art = Gasto`). La UI muestra importe cuando el valor es distinto de cero; si no hay dato, **—**. El **total** consolidado del período **no** se desglosa al árbol: columna **TOTAL** en rubro/subrubro/artículo sigue en **—** (solo el renglón **cliente** lleva el total real). **Objetivo** y **Falta** siguen solo a nivel cliente/vendedor (— en rubro/subrubro/artículo). La UI replica expansión/colapso por vendedor, cliente, rubro y subrubro (persistencia en `localStorage` bajo clave `synap:report-view:ventas-objetivos-vs-bo:jerarquia`). Cabeceras en **mayúsculas**; grupo **VENTAS PERÍODO** con `colspan="5"` sobre **UNIDADES**, **FACTURACIÓN**, **REMITOS**, **PEDIDOS EN ARMADO** y **TOTAL**; columnas **OBJETIVO** (fondo verde suave) y **FALTA** (fondo rojo suave) con `rowspan="2"`; subcabecera **TOTAL** con tono violeta un poco más marcado; grupo **BACKORDER** con **BO TOTAL** al final más oscuro; fila **Totales** resaltada.
  - **Contadores en jerarquía:** junto al nombre del vendedor y del bloque Con compra / Sin compra, el total entre paréntesis `(n)` se muestra en **tipografía ligeramente menor** y **color gris** (`text-slate-600` / `dark:text-slate-400`) para diferenciarlo del nombre manteniendo legibilidad en modo oscuro.
  - **Expansión bajo vendedor:** al expandir un vendedor, las filas **Con compra** y **Sin compra** aparecen **colapsadas** hasta que el usuario las abra (primera vez sin preferencia guardada en `localStorage`). El estado expandido/colapsado de cada una se persiste en la misma clave de jerarquía (`synap:report-view:ventas-objetivos-vs-bo:jerarquia`, nodo `expandedNodes` con clave `ec-{cod_viajante}-con_compra` / `sin_compra`).
  - **Cabecera y scroll:** `thead` sticky en bloque único (como MPR ventana-pack). **Dos filas:** «VENDEDOR / CLIENTE / RUBRO», **OBJETIVO** y **FALTA** con `rowspan="2"`; en la primera fila, **VENTAS PERÍODO** (`colspan="5"`) y **BACKORDER** (`colspan="4"`); en la segunda fila, **UNIDADES**, **FACTURACIÓN**, **REMITOS**, **PEDIDOS EN ARMADO**, **TOTAL** y las cuatro BO. En cada fila del cuerpo, **icono y nombre comparten una celda** (`flex`, `gap-1`): `padding-left` desplaza junto el chevron y el texto (sin columna vacía entre flecha y nombre). Jerarquía vendedor→cliente→rubro→subrubro→artículo con **el mismo incremento** de `padding-left` por nivel; chevrons **▸**/**▾** salvo en artículo (reserva de ancho). Solo **Rubro** y **Subrubro** llevan etiqueta discreta antes del nombre. En la grilla web **no** se muestran códigos numéricos junto al nombre; los códigos siguen en el JSON para toggles y exportación.
  - **Controles de grilla (web):** barra sobre la tabla con **Expandir todo** / **Contraer todo** (misma idea que `/ventas/objetivos-venta/<id>/`), **búsqueda predictiva** (mín. 2 caracteres, filtra por texto de vendedor/cliente/rubro/subrubro/artículo; la fila Totales no se oculta). **Rendimiento:** con búsqueda vacía o de un solo carácter, al expandir/colapsar niveles no se recorre toda la tabla salvo que existan filas con `vo-bo-search-hide`; con búsqueda activa (≥ 2 caracteres) sí se recalcula el filtro para mantener coherentes las cabeceras de vendedor. Limpiar la marca `vo-bo-search-hide` al borrar la búsqueda solo recorre filas que la tienen. **Visualización:** valores numéricos **negativos en rojo** (`text-red-600` / `dark:text-red-400`; para celdas se usa prefijo `!` donde hace falta para que no pierdan contra `text-slate-700` del `<td>`), incluida **FALTA** cuando el valor mostrado es negativo (brecha pendiente) y KPI **Total objetivo** si es negativo; KPI **Falta para el objetivo** en rojo cuando el estado es pendiente (importe mostrado negativo). Las utilidades Tailwind usadas en `reports/static/reports/js/objetivos_ventas_bo.js` deben entrar en el build: en `theme/static_src/tailwind.config.js` el `content` incluye `../../reports/static/**/*.js`. Bloque KPI y tabla separados visualmente (`margin` entre secciones).
  - **KPI «Falta para el objetivo»:** mismo criterio visual que la columna **FALTA**: importe **−(Σ objetivo − facturación − remitos − pedidos en armado)** del backend (**negativo** = pendiente de cumplir; **positivo** = objetivo superado; **cero** = alcanzado). **Rojo** si pendiente; **verde** si alcanzado o superado. Debajo del importe, **Estado: …** aclara la lectura. Cuando la brecha es exactamente **cero**, la UI muestra **`$ 0,00` sin signo menos** (evita el artefacto de **−0** de coma flotante al invertir el signo en JS).
