...
| Visibilidad | Quien tiene acceso gerencial al informe ve **el detalle completo** de la empresa: **sin** restricción obligatoria por sucursal o PV (los filtros, si existen en UI, son **opcionales** para enfocar, no para ocultar datos por defecto). |
...
- **Backend:** endpoint dedicado (p. ej. `GET /api/reports/executive-summary/` o ampliación de `reports-query` con `slug` reservado) que devuelva en un solo payload:
  - `fecha_referencia`, `kpis`, `serie_horaria[]`, `serie_7_dias[]`, `split_mayorista_minorista` (montos y/o %),
  - `meta` con zona horaria aplicada y versión de definición.
