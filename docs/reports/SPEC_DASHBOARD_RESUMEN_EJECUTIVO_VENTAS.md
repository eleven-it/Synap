...
| **Margen bruto del día** | Sobre renglones **`stock`** del mismo día y filtros que **Unidades vendidas** / Top 10: suma de `PrecioNetoxR` (**venta neta líneas**) y `PrecioCostoxR` (**costo neto líneas**) con signo FA/NC. **Paridad informe rentabilidad AdministraNET** (`venta_rentabilidad_resumen`, Crystal `ventas_vista_rentabilidad_*.rpt`): costo = valor persistido en renglón, sin escala Display/Bulto. API: `meta.margen_costo_criterio` = `precio_costoxr_linea`. |
