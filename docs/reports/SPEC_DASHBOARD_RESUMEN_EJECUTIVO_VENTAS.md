...
| **Margen bruto del día** | Sobre renglones **`stock`** del mismo día y filtros que **Unidades vendidas** / Top 10: suma de `PrecioNetoxR` (**venta neta líneas**) y `PrecioCostoxR` (**costo neto líneas**) con signo FA/NC. **Margen absoluto** = venta neta líneas − costo neto líneas; **%** sobre venta neta líneas si el denominador ≠ 0 (si no, `null` en JSON). Costo del **renglón de comprobante**, no `articulo.PrecioCosto` actual. |
| **Margen por rubro / subrubro** | Agregación por `articulo` → `rubro` y `subrubro`; orden por venta neta del grupo descendente; sin clasificación → **«Sin clasificar»**. |
...
- **Backend:** endpoint dedicado (p. ej. `GET /api/reports/executive-summary/` o ampliación de `reports-query` con `slug` reservado) que devuelva en un solo payload:
  ...
  - `margen_bruto` (totales día: `venta_neta_lineas`, `costo_neto_lineas`, `margen_absoluto`, `pct_sobre_venta_lineas`), `margen_por_rubro[]`, `margen_por_subrubro[]`; en `meta`: `definicion` = `executive-sales-v2`, `margen_costo_criterio`, `margen_venta_criterio`, `nota_venta_neta_lineas_vs_comprobante` (diferencia KPI comprobante vs suma de líneas).
...
| Front | `reports/static/reports/js/executive_summary.js` (d3 v7 por CDN); select **Sucursal** (todas + opciones desde API) y **Orden Top 10** (venta neta / unidades); bloque **Top 10** responsive; KPI «Vs ayer» con % + gap en $. **Rentabilidad del día:** KPIs venta/costo/margen por líneas y tablas rubro/subrubro. |
...
**Implementado (11/05/2026):** bloque **Rentabilidad del día** (UI): margen bruto total y tablas por rubro y subrubro; contrato API ampliado (`openspec/changes/resumen-gerencial-margen-bruto/`). Nota: **`ventas_netas_dia`** (comprobante) puede diferir de **`margen_bruto.venta_neta_lineas`**; el % de margen del bloque rentabilidad es sobre **venta neta de líneas** del universo `stock` definido.
