...
| **Margen bruto del día (planificado)** | Sobre el **mismo universo de renglones** que Top 10 / unidades: suma de `stock.PrecioNetoxR` (**venta neta líneas**) y suma de `stock.PrecioCostoxR` (**costo neto líneas**) con signo FA/NC. **Margen absoluto** = venta neta líneas − costo neto líneas; **%** = margen / venta neta líneas si el denominador ≠ 0. **No** recalcular con `articulo.PrecioCosto` actual en la v1. |
| **Margen por rubro / subrubro (planificado)** | Agregación por `articulo.CodigoRubro` → `rubro` y por `articulo.IDSubRubro` → `subrubro`; orden por venta neta del grupo descendente; renglones sin artículo o sin clasificación → bucket «Sin clasificar». |
...
- **Backend:** endpoint dedicado (p. ej. `GET /api/reports/executive-summary/` o ampliación de `reports-query` con `slug` reservado) que devuelva en un solo payload:
  ...
  - **(Planificado — cambio `resumen-gerencial-margen-bruto`):** `margen_bruto` (totales día), `margen_por_rubro[]`, `margen_por_subrubro[]`; en `meta`: `margen_costo_criterio`, `margen_venta_criterio` y bump de `definicion` si aplica (p. ej. `executive-sales-v2`).
...
| Front | `reports/static/reports/js/executive_summary.js` (d3 v7 por CDN); select **Sucursal** (todas + opciones desde API) y **Orden Top 10** (venta neta / unidades); bloque **Top 10** responsive; KPI «Vs ayer» con % + gap en $. |
...
**Planificación (11/05/2026, modo híbrido):** rentabilidad — **margen bruto total** y **por rubro/subrubro** (`openspec/changes/resumen-gerencial-margen-bruto/`). Nota: **`ventas_netas_dia`** (comprobante) puede diferir de la suma de **`PrecioNetoxR`** en líneas; el % de margen del bloque rentabilidad es sobre **venta neta de líneas** del universo `stock` definido.
