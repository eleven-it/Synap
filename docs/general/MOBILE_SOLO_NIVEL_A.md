...
- `/contabilidad/cotizacion-dolar/` — pantalla vigente / sugerido BCRA / aceptar / manual / historial (Synap + modales; layout responsive &lt; `lg`).
- APIs: prefijo `/contabilidad/api/cotizacion/` (vigente, sugerencia, aceptar, manual, historial).

Menú PWA contabilidad: solo `contabilidad_cotizacion_dolar` si el usuario tiene `contabilidad.cotizacion.ver` (`PWA_CONTABILIDAD_DEEP_LINKS` en `core/pwa_nivel_a.py`).

**QA device:** checklist §G en [docs/reports/QA_VMM_PWA_P7.md](../reports/QA_VMM_PWA_P7.md) — **pendiente ejecución en dispositivo**.
...
- **`apps_visibles_para_usuario`** (`core/utils/utils.py`): tras resolver permisos y reglas habituales, se aplica `filtrar_apps_menu_para_pwa_movil` (`core/pwa_nivel_a.py`). En móvil solo permanecen entradas cuyo `id` está en `PWA_MENU_APP_IDS` (**`self_checkout`**, **`ecom`**, **`stock`**, **`mpr`** y **`contabilidad`** (solo cotización dólar), cada uno sujeto a permisos de menú de escritorio). TPV requiere `usuario_tiene_tpv_en_menu`; e-com requiere `usuario_tiene_ecom_en_menu` y filtra submenús a `ecom_compra` (pedido simple → masivo `?modo=simple`), `ecom_pedidos` (hub) y `ecom_pedido_masivo` (masivo); stock conteo requiere `stock.inventario_fisico.contar` y filtra a `stock_inv_fisico_conteo`; MPR requiere `usuario_tiene_mpr_en_menu` y filtra submenús a `mpr_prod_kpis` (tablero KPIs → `/mpr/`) y `mpr_prod_inventario` (inventario → `/mpr/inventario/`); contabilidad requiere `contabilidad.cotizacion.ver` y filtra a `contabilidad_cotizacion_dolar` → `/contabilidad/cotizacion-dolar/`. Deep links: `PWA_ECOM_DEEP_LINKS`, `PWA_MPR_DEEP_LINKS` y `PWA_CONTABILIDAD_DEEP_LINKS` en `core/pwa_nivel_a.py`; e-com también en `deep_link` / `nivel_a` en `ecom/menu_config.py` (`ecom_compra` → `/ecom/mayoristapp/pedido-masivo-sucursales/?modo=simple`).
