...
**Búsqueda predictiva (módulo stock):** `core/services/administranet_stock.py` — `get_stock_por_lote(base_empresa, id_articulo, id_deposito)` usa esta tabla con JOIN a `lote` para mostrar stock por lote en el ingreso de renglón de movimiento de stock. Véase [BUSQUEDA_PREDICTIVA_ARTICULO_MOVIMIENTO_STOCK.md](../../BUSQUEDA_PREDICTIVA_ARTICULO_MOVIMIENTO_STOCK.md).
