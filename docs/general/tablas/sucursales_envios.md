...
## 4. Bug conocido en AdministraNET (VB6)

En AdministraNET el listado de tipos de envío por zona **no filtra por la sucursal en edición**: se muestra el mismo conjunto de registros para todas las sucursales (los que en la tabla tienen `id_sucusal` de una sola sucursal). Comportamiento observado: datos cargados para una sucursal (ej. id_sucusal=4) aparecen al abrir cualquier sucursal en el ABM.

## 5. Uso en Synap (core)

En Synap el listado de “Tipo de cobro por envío” **sí filtra por sucursal**: se usa el `id_sucursal` de la URL de edición (branch) y la columna `id_sucusal` en la tabla. Cada sucursal ve solo sus propios registros. Paridad correcta con el modelo de datos; no se replica el bug de AdministraNET.
