...
### 4.1 ABM Sucursales en Synap (core)

Synap expone la pantalla **Sucursales** (menú Archivo → Parámetros): lista, alta y edición sobre esta tabla. **Comportamiento:** las sucursales no se eliminan; se desactivan con el campo `anulado` (toggle Activa/Anulada desde la lista). El formulario incluye bloques Datos, COT (cot_*), Geolocalización (geo_*) y Envíos (activa_calculo_envios). Servicio: `core/services/administranet_sucursales.py`; vistas en `core/views/views.py`. Detalle: [AVANCES_MIGRACION_ARCHIVO.md](../AVANCES_MIGRACION_ARCHIVO.md).
