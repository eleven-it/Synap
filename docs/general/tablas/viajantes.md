...
## 5. Listado centralizado (desplegables)

Para **listar vendedores (viajantes) no anulados** en desplegables (sucursal, kiosco, TPV táctil) se usa una única implementación en core:

- **Función canónica:** `AdministraNETSucursalesService().obtener_viajantes(base_empresa)` en **core/services/administranet_sucursales.py**.
- **Consulta:** `SELECT CodViajante, COALESCE(Nombre, '') AS Nombre FROM viajantes WHERE COALESCE(anulado, 'No') = 'No' ORDER BY Nombre` (paridad con Configuración.frm).
- **Quién la usa:**
  - **Core:** formulario de sucursal (Opciones Generales → Vendedor).
  - **Self Checkout:** `listar_viajantes(base_empresa)` en `self_checkout/services/config_options_service.py` delega en el servicio de core; usada por la configuración de kiosco y por la API de vendedores para el TPV táctil.

No duplicar la consulta en otros módulos; reutilizar el servicio de core o la función `listar_viajantes` de self_checkout (que ya delega en core).
