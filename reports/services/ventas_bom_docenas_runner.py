...
def _resolve_period(filters: dict, resolve_period_dates) -> Tuple[Optional[str], Optional[str]]:
    # Compatibilidad con payloads que usan nombres de facturación.
    if filters.get("fecha_inicio_facturacion") and not filters.get("fecha_inicio"):
        filters = {**filters, "fecha_inicio": filters.get("fecha_inicio_facturacion")}
    if filters.get("fecha_fin_facturacion") and not filters.get("fecha_fin"):
        filters = {**filters, "fecha_fin": filters.get("fecha_fin_facturacion")}
    return resolve_period_dates(filters)
