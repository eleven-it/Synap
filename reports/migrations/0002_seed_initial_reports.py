...
def create_report_definitions(apps, schema_editor):
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if not cursor.fetchone():
                ...
                print("⚠️  Tabla reports_reportdefinition no existe, saltando seed de reportes")
                print("   💡 Ejecuta primero: python manage.py migrate reports 0001_initial")
                return
    except Exception as e:
        # Si hay error verificando, intentar continuar (puede ser que la tabla exista)
    ...
    try:
        ...
        print(f"⚠️  Error obteniendo modelos: {e}, saltando seed de reportes")
        return
    ...
    for definition in definitions:
        try:
            ...
            # Si hay error (tabla no existe, etc.), continuar con el siguiente
            print(f"⚠️  Error creando reporte {definition.get('slug', 'unknown')}: {e}")
            continue


def delete_report_definitions(apps, schema_editor):
    """Elimina los reportes creados por create_report_definitions"""
    ...
    try:
        with connection.cursor() as cursor:
            ...
            """)
            if not cursor.fetchone():
                # La tabla no existe, no hay nada que eliminar
                print("⚠️  Tabla reports_reportdefinition no existe, saltando eliminación de reportes")
                return
    except Exception as e:
    ...
    try:
        ...
        print(f"⚠️  Error obteniendo modelo: {e}, saltando eliminación")
        return
    ...
    try:
        ...
        print(f"⚠️  Error eliminando reportes: {e}")
