      ...
      (enter) => {
        ...
        groupKeys.forEach((groupKey, groupIndex) => {
          ...
          const text = g.append("text")
            ...
            .attr("class", "text-[9px] font-semibold")
            .style("opacity", 0)
            .text((d) => {
...
const renderCards = (container, data, config) => {
  ...
  orderedFields.forEach((field, index) => {
    ...
    if (isTotalConsolidado) {
      subtitle = `<p class="text-[10px] sm:text-[11px] text-slate-300 opacity-90 mt-2.5 sm:mt-3 text-center font-medium">VENTAS NETAS + REMITOS NO FACTURADOS + PEDIDOS PENDIENTES</p>`;
    }
    
    // Estilos diferenciados para TOTAL CONSOLIDADO
    const baseClasses = "flex flex-col justify-center rounded-xl sm:rounded-2xl text-white px-3 sm:px-4 py-4 sm:py-6";
    const cardClasses = isTotalConsolidado
      ? `${baseClasses} bg-gradient-to-br from-purple-700 via-purple-800 to-purple-900 dark:from-purple-800 dark:via-purple-900 dark:to-purple-950 shadow-2xl shadow-purple-500/40 dark:shadow-purple-600/30 ring-2 ring-purple-400/30`
      : `${baseClasses} bg-slate-900/90 dark:bg-slate-800/90 shadow-lg`;
    
    const labelClasses = isTotalConsolidado
      ? "text-xs sm:text-sm uppercase tracking-[0.2em] sm:tracking-[0.3em] text-purple-100 mb-1.5 sm:mb-2 leading-tight font-semibold"
      : "text-xs sm:text-sm uppercase tracking-[0.2em] sm:tracking-[0.3em] text-slate-400 mb-1.5 sm:mb-2 leading-tight";
    
    const valueClasses = isTotalConsolidado
      ? "text-xl sm:text-2xl font-bold break-words text-white"
...
const renderSummary = (meta, totals) => {
  ...
  const order = ["saldo_inicial", "operating_flow", "investing_flow", "financing_flow", "cash_variation", "saldo_final", "total_subtotal_desc", "ventas_netas", "remitos_no_facturados", "pedidos_pendientes", "total_consolidado"];
  ...
  if (ventasNetasCard) {
    // Para ventas_netas: alineada verticalmente con VENTAS NETAS
    const lastUpdateContainer = document.createElement("div");
    lastUpdateContainer.id = "last-update-time";
...
const loadWorkspaceSlot = async (slot, index, isAutoRefresh = false) => {
  ...
  try {
    // Para reportes que requieren filtros de fecha, siempre usar mes en curso en workspace
    ...
    if (reportsWithDateFilters.includes(slot.slug)) {
      ...
      // Calcular fecha de inicio: 60 días atrás desde hoy
      const firstDay = new Date(today);
      firstDay.setDate(today.getDate() - 60);
      // Fecha fin: hoy
...
const initializeWorkspaceRealtime = () => {
  ...
  if (savedRealtimeState === "true") {
    ...
    startWorkspaceRealtime(getRefreshIntervalMs(currentInterval));
  }
...
if (dashboardRoot) {
  ...
  const initializeRealtime = () => {
    ...
    // Obtener refresh_interval del filtro o del reporte desde el DOM
    const getCurrentRefreshInterval = () => {
      const refreshIntervalSelect = document.getElementById("refresh_interval");
      if (refreshIntervalSelect && refreshIntervalSelect.value) {
        return refreshIntervalSelect.value;
      }
      return dashboardRoot?.dataset.reportRefreshInterval || "daily";
    };
    ...
    const updateRealtimeUI = (active) => {
      ...
      if (active) {
        ...
        realtimeButton.classList.add("text-slate-400", "hover:text-slate-300");
        if (label) label.textContent = "Tiempo real";
        if (indicator) {
          indicator.classList.add("opacity-0");
          indicator.classList.remove("opacity-100", "animate-pulse");
    ...
    if (savedRealtimeState === "true") {
      ...
      const currentInterval = getCurrentRefreshInterval();
      startRealtime(getRefreshIntervalMs(currentInterval));
      }
    }
