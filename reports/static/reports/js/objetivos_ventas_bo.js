...
(function () {
  ...
  const tdNeg = "text-rose-600 dark:text-rose-400 font-medium";

  function faltaClass(falta) {
    const n = Number(falta);
    if (!Number.isNaN(n) && n < 0) return " " + tdNeg;
    if (!Number.isNaN(n) && n > 0) return " text-amber-700 dark:text-amber-300 font-medium";
    return "";
  }
  ...
  const kpiFaltaCardPendiente =
    "rounded-2xl border border-rose-200 bg-rose-50/70 p-6 text-left shadow-lg shadow-rose-900/10 transition-colors dark:border-rose-900/55 dark:bg-rose-950/40 dark:shadow-black/20";
  ...
  /** Fila con todas las métricas (vendedor / cliente / totales). Orden: Objetivo, Falta, Unidades, Facturación, Remitos, Total, BO c/stock, BO c/ingreso, BO s/stock, BO total */
  function metricCellsFull(row) {
    return (
      `<td class="${tdNum} vo-td-obj ${tdObj}${negMoneyClass(row.objetivo)}">${fmtMoney(row.objetivo)}</td>` +
      `<td class="${tdNum} vo-td-falta ${tdFaltaBody}${faltaClass(row.falta)}">${fmtMoney(row.falta)}</td>` +
  ...
  function renderKpis(totals) {
    ...
    elObj.className =
      ...
      (Number.isFinite(objN) && objN < 0 ? "text-rose-600 dark:text-rose-400" : "text-slate-900 dark:text-white");
    ...
    const absFal = Math.abs(falN);
    elFalta.textContent = fmtMoney(absFal);
    elFaltaEstado.classList.remove("hidden");

    if (falN > 0) {
      elFalta.className = valorClsBase + "text-rose-700 dark:text-rose-300";
      elFaltaEstado.textContent =
        "Estado: pendiente. La suma de objetivos aún supera a facturación más remitos; falta cubrir el importe mostrado (valor absoluto de la brecha).";
      elFaltaEstado.className = estadoBase + "text-rose-800 dark:text-rose-200";
      elFaltaCard.className = kpiFaltaCardPendiente;
      setSvgOrElementClass(elFaltaIcon, "w-5 h-5 text-rose-600 opacity-90 dark:text-rose-400");
    } else {
      elFalta.className = valorClsBase + "text-emerald-700 dark:text-emerald-300";
      if (falN < 0) {
        elFaltaEstado.textContent =
          "Estado: objetivo superado. Facturación más remitos superan la suma de objetivos; la cifra es el margen a favor en valor absoluto.";
      } else {
        elFaltaEstado.textContent =
          "Estado: objetivo alcanzado. Facturación más remitos igualan la suma de objetivos (brecha cero).";
      }
  ...
  function buildThead() {
    ...
    return (
      ...
      `<th rowspan="2" class="${thRs2Base} ${thObj} text-right text-emerald-950 dark:text-emerald-100">OBJETIVO</th>` +
      `<th rowspan="2" class="${thRs2Base} ${thFaltaHdr} text-right text-rose-950 dark:text-rose-100">FALTA</th>` +
      `<th colspan="4" class="${thGrpVentas} px-2 py-1.5 text-center align-bottom">VENTAS PERÍODO</th>` +
        ...
        parts.push(
          `<tr class="${rowHover} ${hideSub ? "hidden" : ""} bg-white dark:bg-slate-900/20"${voSearchDataAttr(ss)} data-vo-vendor-group="${vg}" data-parent="${escHtml(rg)}" data-vo-under-client="${escHtml(cid)}" data-vo-under-rubro="${escHtml(rg)}" data-vo-sub-key="${escHtml(sg)}">` +
            treeNombreCell(
              ...
              "text-xs uppercase font-bold tracking-tight text-slate-800 dark:text-slate-100",
              `${etiquetaJerarquia(3)}${escHtml(sub.nombre_subrubro)}`
            ) +
            metricCellsVentaSolo(sub) +
            "</tr>"
        );
        (sub.children || [])
          .filter(function (art) {
  ...
  function wireNestedToggles(container) {
    container.querySelectorAll("[data-vo-chev]").forEach(function (chev) {
      ...
      function toggleFromChev() {
        ...
        if (!direct.length && String(gid).indexOf("c-") !== 0) return;
  ...
  function renderTable(jerarquia, totals) {
    ...
    jerarquia.forEach((vend) => {
      ...
      (vend.children || [])
        ...
        const cliSearch = [vend.nombre_vendedor, codViajante, cli.nombre_cliente, cid]
          .filter(function (x) {
            return x != null && String(x).trim() !== "";
          })
          .join(" ")
          ...
          .trim();
        parts.push(
          `<tr class="vo-child-row hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors ${expanded ? "" : "hidden"} bg-white dark:bg-slate-900/30"${voSearchDataAttr(cliSearch)} data-parent="${escHtml(gid)}" data-vo-vendor-group="${escHtml(gid)}" data-vo-client="${escHtml(cid)}">` +
            treeNombreCell(
              treeIndentPx(1),
              chevC,
              "text-xs font-bold uppercase tracking-tight text-slate-800 dark:text-slate-100",
        ...
        if (hasDet) {
          appendVentaDetalle(parts, cli.venta_detalle, gid, cg, cid, viewState, expanded, expC, cliSearch);
        }
      });
    });
