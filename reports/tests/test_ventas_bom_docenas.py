...
class AggregateBomTests(TestCase):
    def test_aggregate_and_omit_packs_without_bom(self):
        pack_rows = [
            ...
            {"id_en_abm": 2, "qty_pack": 3},  # sin BOM
        ]
        bom = {
            1: [
    ...
    def test_fa_minus_nc(self):
        ...
        self.assertEqual(pares[9], 24.0)  # (10-2)*3
...
class ExportVentasBomTests(TestCase):
    def test_filename_uses_dates(self):
        ...
        name = svc._resolve_export_filename(
            VENTAS_BOM_DOCENAS_SLUG,
            {"filters": {"fecha_inicio": "2026-03-01", "fecha_fin": "2026-03-31"}},
            "20260313_120000",
        )
        self.assertEqual(name, "Ventas_BOM_docenas_01032026_31032026.xlsx")

    def test_filter_lines_period_ddmmyyyy(self):
        svc = ExportService(user=SimpleNamespace())
        qr = SimpleNamespace(meta={"fecha_inicio": "2026-01-15", "fecha_fin": "2026-01-20"}, notes=[])
        lines = svc._ventas_bom_filter_lines(
            {"fecha_inicio": "2026-01-15", "fecha_fin": "2026-01-20", "sucursales": [1, 2]},
            qr,
        )
        self.assertEqual(lines[0], ("Período", "15/01/2026 — 20/01/2026"))
        self.assertEqual(lines[1][0], "Sucursales")
