...
@patch(
    "reports.services.executive_sales_summary.fetch_utiliza_embalaje_display_bulto",
    return_value=True,
)
class ExecutiveSummaryContractTests(SimpleTestCase):
    def test_run_executive_summary_con_cursor_vacio_devuelve_estructura(self, _mock_emb):
        cursor = MagicMock()
        ...
        self.assertEqual(out["meta"].get("margen_costo_criterio"), "costo_empaque_escala_cantidad_dividir")
        self.assertTrue(out["meta"].get("utiliza_embalaje_display_bulto"))
        self.assertEqual(out["meta"].get("margen_venta_criterio"), "precio_netoxr_linea")
        self.assertIn("nota_margen_costo_historico", out["meta"])
    ...
    def test_run_executive_summary_propaga_filtro_sucursal_y_orden_top(self, _mock_emb):
        cursor = MagicMock()
        cursor.execute = MagicMock(return_value=None)
        cursor.fetchone = MagicMock(return_value=(0.0, 0.0))
        cursor.fetchall = MagicMock(return_value=[])
    ...
    def test_margen_pct_sobre_venta_lineas_cuando_hay_venta(self, _mock_emb):
...
class ExecutiveSummaryEmbalajeConfigTests(SimpleTestCase):
    @patch(
        "reports.services.executive_sales_summary.fetch_utiliza_embalaje_display_bulto",
        return_value=False,
    )
    def test_meta_criterio_costo_sin_embalaje(self, _mock_emb):
        cursor = MagicMock()
        cursor.execute = MagicMock(return_value=None)
        cursor.fetchone = MagicMock(return_value=(0.0, 0.0))
        cursor.fetchall = MagicMock(return_value=[])
        ...
        out = run_executive_summary(cursor, date(2026, 4, 1), [], [])
        self.assertFalse(out["meta"].get("utiliza_embalaje_display_bulto"))
        self.assertEqual(
            out["meta"].get("margen_costo_criterio"),
