...
def fix_product_price(content):
    # Agregar price=Decimal('100.00') si no está presente
    pattern = re.compile(r'Product\.objects\.create\(([^)]*)\)')
    def repl(match):
        args = match.group(1)
        if 'price=' not in args:
            if args.strip():
                return f"Product.objects.create({args}, price=Decimal('100.00'))"
            else:
                return "Product.objects.create(price=Decimal('100.00'))"
...
for file_path in files:
    if os.path.exists(file_path):
        ...
        content = fix_product_price(content)
...
print("Corrección de price en Product completada") 