...
| 4 | Validación importes | Por FA: Σ `Cantidad×PrecioNetoxU` vs `SubTotal1` (predescuento), y Σ `Cantidad×(PrecioNetoxU+PrecioIVAxU)×(SubtotalDesc/SubTotal1)` vs `ImporteVenta`. Tolerancia `max(0.05, 0.01×n_lineas)`. Fuera de tolerancia ⇒ `errores[]`, **export 409**; preview muestra el error | Igualdad exacta; comparar bruto de líneas sin factor de descuento | FA 1200 (31 líneas) difiere 0,02 en IVA por redondeo. FA DABRA con `PorDesc1=20` (ej. 1367): las líneas quedan predescuento y `ImporteVenta` ya descontado ⇒ sin el factor `SubtotalDesc/SubTotal1` el export queda bloqueado con falsos positivos |
| 5 | Bonificación | `pordesc_bonif`≠0 else `PorDesc` de línea; si ambos 0 → % pie `(SubTotal1−SubtotalDesc)/SubTotal1×100`. Importe/IVA línea × factor cabecera; `TotalGravado`/`Imp Neto` = `SubtotalDesc` | Sólo descuento de línea; `TotalGravado=SubTotal1` | En DABRA julio 2026, 39/59 FA tienen `PorDesc1=20` y **0** líneas con `pordesc_bonif`/`PorDesc` ≠0: sin fallback a pie, Bonificacion/ImporteBonificacion salían 0 y TotalGravado ignoraba el descuento |
