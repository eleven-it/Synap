# Spec: Informe utilidad gerencial

**Change:** `informe-utilidad-gerencial`
**Artifact Type:** delta spec (capability `reports-utilidad-gerencial`)
**Fuente PHP:** `relay-ventas-netas-gerencia.php` (modo `verInforme=ut` → `utilidades_totales_todos`; `uti` → `utilidades_totales_todos_inflacion`)
**Target Synap:** `reports/` (servicio + relay API + ReportDefinition `utilidad-gerencial`)
