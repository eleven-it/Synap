# Delta for ecom-vendedor-operativo

**Change:** `ecom-pedidos-usabilidad-supervisor`  
**Tipo:** spec completa (capability nueva)
...
El sistema MUST cargar `vendedor_a_cargo` desde MySQL legacy al hidratar sesión mayoristapp cuando `supervisor_venta='Si'`. MUST normalizar la lista a enteros (`CodViajante`) válidos.

#### Scenario: Supervisor con cartera en legacy

- **GIVEN** usuario con `supervisor_venta='Si'` y vendedores `[20, 21]` en legacy
...
Si `supervisor_venta='Si'` y `vendedor_a_cargo` no está vacío, el sistema MUST exponer selector de vendedor operativo en flujos de pedido (simple y masivo). La opción MUST incluir al propio supervisor y cada `CodViajante` de su cartera.
