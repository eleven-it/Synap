...
- [x] 3.7 Cutover local/dev: `SYNAP_PERMISOS_SOURCE=synap` confirmado en `.env`. Toggle/atajo NO tocan `permiso_sistema_puesto`.
...
- [x] 4.2 Eliminar `core/services/sync_permisos_synap.py` y referencias (`sync_synap_permissions_to_adminet`, `bootstrap_instalacion` → `apply_synap_permisos_tables`).
- [x] 4.3 Establecer `SYNAP_AUTO_SYNC_PERMISSIONS=False` por defecto; `.env` local alineado. `SYNAP_AUTO_ENSURE_SCHEMA=True` intacto.
- [x] 4.4 Ejecutar purge en empresas con backfill validado y `SYNAP_PERMISOS_SOURCE=synap` estable — **local `administranet` ejecutado** (`purge_synap_legacy_permisos --ejecutar`); **WARNING:** purge remoto MySQL (`192.168.0.2`) pendiente cuando el host sea alcanzable.
