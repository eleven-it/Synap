# Especificación — Informe cobranzas por vendedor (Synap)

Fuente de verdad OpenSpec para el slug `cobranzas-por-vendedor`, servicio `get_cobranzas_vendedor` y relays `/api/reports/cobranzas-vendedor/relay/` (operativo) y `/api/reports/cobranzas-vendedor/relay/gerencia/` (gerencial).

Incorporado desde el cambio archivado `informe-cobranzas-por-vendedor` (02/07/2026). Fuente PHP: `listado-cobranzas-vendedor.php`, `informes-json/cobranza_lista_vendedor_resumen.php`. Patrón de referencia: `informe-clientes-sin-ventas-vendedor`.
...
- **Archivado:** 13/07/2026 (`openspec/changes/archive/2026-07-13-informe-cobranzas-por-vendedor/`)
- **Estado:** vigente (implementado y verificado)
- **Requisitos:** 7 · **Escenarios:** 11
