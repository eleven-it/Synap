-- Ternas Vendedor → Cliente → Marca (territorio comercial ecom).
...
CREATE TABLE IF NOT EXISTS ecom_vendedor_cliente_marca (
    ...
    -- Unique parcial: solo filas activas (anulado=No). Varias anuladas pueden repetir cliente+marca.
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_evcm_cliente_marca_activo (id_cliente, CodMarca, anulado_activo),
    KEY idx_evcm_viajante (CodViajante),
    KEY idx_evcm_cliente (id_cliente),
    KEY idx_evcm_marca (CodMarca)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
COMMENT='Terna vendedor-cliente-marca (pedido masivo / filtro catálogo)';
