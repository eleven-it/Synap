"""
APIs nota de crédito mayoristapp (``relay_nota_credito.php``).
"""
...
from ecom.services.nota_credito_relay import (
    listar_nota_credito_relay,
    sugerencias_nro_nota_credito_relay,
)
...
class NotaCreditoListadoRelayAPIView(APIView):
    """
    ...
    def post(self, request: Request) -> Response:
        ...
        rows = listar_nota_credito_relay(base, dict(request.data), _session_user(request), idc, limit=lim)
        return Response({"total": len(rows), "filas": rows})


class NotaCreditoSugerenciasNroRelayAPIView(APIView):
    """
    ...
    def get(self, request: Request) -> Response:
        ...
        idc = leer_idcliente_mayoristapp(request)
        nums = sugerencias_nro_nota_credito_relay(base, q, _session_user(request), idc)
