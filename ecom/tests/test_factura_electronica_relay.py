# Tests APIs factura electrónica mayoristapp.
...
class TestFacturaElectronicaListadoView(unittest.TestCase):
    def _user(self):
        u = MagicMock()
    ...
    @patch("ecom.factura_electronica_relay_views.listar_factura_electronica_relay")
    def test_post_ok(self, mock_listar):
        mock_listar.return_value = [{"CodigoMovimiento": 1}]
        req = _req_post(
            "/ecom/api/mayoristapp/fe/factura-electronica/listado/?ajax=1",
            {"campoBusca": "-"},
            {"base_empresa": "emp1"},
        )
        force_authenticate(req, user=self._user())
        resp = FacturaElectronicaListadoRelayAPIView.as_view()(req)
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.data["total"], 1)


class TestFacturaElectronicaSugerenciasView(unittest.TestCase):
    def _user(self):
        u = MagicMock()
        u.is_authenticated = True
        u.is_superuser = False
    ...
    @patch("ecom.factura_electronica_relay_views.sugerencias_nro_factura_electronica_relay")
    def test_get_ok(self, mock_s):
        mock_s.return_value = ["100", "1001"]
        req = _req_get(
            "/ecom/api/mayoristapp/fe/factura-electronica/sugerencias-nro/",
            {"ajax": "1", "q": "10"},
            {"base_empresa": "emp1", "tipousuario": "vendedor", "id_vendedor_usr": 1},
        )
        force_authenticate(req, user=self._user())
        resp = FacturaElectronicaSugerenciasNroRelayAPIView.as_view()(req)
        self.assertEqual(resp.status_code, 200)
