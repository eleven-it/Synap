...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for doc in docs:
            ...
            for nombre_rol in roles_lista:
                rol, creado_rol = Rol.objects.get_or_create(
                    nombre__iexact=nombre_rol,
                    defaults={"nombre": nombre_rol, "descripcion": f"Rol importado desde Firebase: {nombre_rol}"}
                )
                usuario.roles.add(rol)
                if rol.nombre.lower() == "administrador":
                    is_admin = True
            ...
            try:
                ...
                print(f"❌ Error guardando usuario {email}: {e}")
                continue
        ...
        for uid in uids_faltantes:
            ...
            print("   [1] Eliminar de la base de datos")
            print("   [3] Saltar")
            opcion = input("   Ingresá tu elección [1/3]: ").strip()
