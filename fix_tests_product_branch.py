...
def fix_product_branch(content):
    ...
    def repl(match):
        ...
        if 'branch=' not in args:
            if args.strip():
                return f"Product.objects.create({args}, branch=self.branch)"
            else:
                return "Product.objects.create(branch=self.branch)"
...
for file_path in files:
    if os.path.exists(file_path):
        ...
        content = fix_product_branch(content)
...
print("Corrección de branch en Product completada") 